...
| XML documents as a panel | done | **Beyond mc**. Same mechanism: elements are containers, attributes are `@name` rows, text and comments are `#text` and `#comment` leaves, `[n]` where siblings repeat |
